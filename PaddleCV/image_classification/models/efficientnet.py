...
import warnings
import os
from .layers import norm_layer, conv2d, init_batch_norm_layer, init_fc_layer
...
class EfficientNet():
    def __init__(self, name='b0', override_params=None, is_test=False):
    ...
    def mb_conv_block(self, inputs, block_args, is_test=False, drop_connect_rate=None, name=None):
        ...
        if id_skip and block_args.stride == 1 and input_filters == output_filters:
            ...
            conv = fluid.layers.elementwise_add(conv, inputs)  # skip connection
    ...
    def extract_features(self, inputs, is_test):
        ...
        # Stem
        #tmp1, tmp = self._conv_stem_norm(inputs, is_test=is_test)
...
def EfficientNetB0(is_test=False):
    model = EfficientNet(name='b0', is_test=is_test)
    return model


def EfficientNetB1(is_test=False):
    model = EfficientNet(name='b1', is_test=is_test)
    return model


def EfficientNetB2(is_test=False):
    model = EfficientNet(name='b2', is_test=is_test)
    return model


def EfficientNetB3(is_test=False):
    model = EfficientNet(name='b3', is_test=is_test)
    return model


def EfficientNetB4(is_test=False):
    model = EfficientNet(name='b4', is_test=is_test)
    return model


def EfficientNetB5(is_test=False):
    model = EfficientNet(name='b5', is_test=is_test)
    return model


def EfficientNetB6(is_test=False):
    model = EfficientNet(name='b6', is_test=is_test)
    return model


def EfficientNetB7(is_test=False):
    model = EfficientNet(name='b7', is_test=is_test)
    return model